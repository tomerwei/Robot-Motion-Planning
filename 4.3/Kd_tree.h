...
private:
	Point q;
public:
  Compare_distance_to_point(const Point& q_)
	  :q(q_) 
  {}
  bool operator()(const typename Point& p1, 
                  const typename Point& p2) const 
  {
...
public:
  ...
  Point nearest_neighbor(const Point& q) 
  {
    ...
    Kernel::FT sq_dist = CGAL::squared_distance(nn, q);
    for (unsigned int i(trees_indx); i < trees.size(); ++i)
    {
      Point tmp_nn = nearest_neighbor(trees[i], q);
      Kernel::FT tmp_sq_dist = CGAL::squared_distance(tmp_nn, q);
...
private:
  ...
  {
    Kernel::FT sq_d = CGAL::squared_distance(points_vec.front(), q);
    Point nn = points_vec.front();
    
    for (unsigned int i(1); i < points_vec.size(); ++i)
    {
      Kernel::FT tmp_sq_d = CGAL::squared_distance(q, points_vec[i]);
...
{
private:
  typedef typename Kd_tree_<Kernel, 
                            typename Kernel::Point_3, 
